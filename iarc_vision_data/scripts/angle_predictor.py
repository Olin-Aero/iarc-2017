...
class AnglePredictor(object):
    ...
    def get_input_shape(self):
        """ gets the input shape of the model for image size checking """
        self.input_shape = tuple([getattr(dim, 'value') for dim in self.model.input.shape.dims[1:]])
    ...
    def predict_angle(self, img):
        ...
        if img.shape != self.input_shape:
            img = imresize(img, self.input_shape[:2])
        if len(img.shape) == 3:
